...
// FXAA_REDUCE_MUL: Try values like 1.0/4.0 or 1.0/16.0.
// FXAA_REDUCE_MIN: Experiment with values like 1.0/64.0 or 1.0/256.0.
// FXAA_SPAN_MAX: Test with values like 4.0 or 12.0.
...
void main()
{
    ...
    if (resolution.x > 600) {
        ...
        // vec3 rgb[5];
        // rgb[0] = texture(texture0, fragTexCoord).xyz; // Center
        // rgb[1] = texture(texture0, fragTexCoord + vec2(-1.0, -1.0) * inverseResolution).xyz; // NW
        // rgb[2] = texture(texture0, fragTexCoord + vec2(1.0, -1.0) * inverseResolution).xyz;  // NE
        // rgb[3] = texture(texture0, fragTexCoord + vec2(-1.0, 1.0) * inverseResolution).xyz;  // SW
        // rgb[4] = texture(texture0, fragTexCoord + vec2(1.0, 1.0) * inverseResolution).xyz;   // SE

        // vec3 lumaCoeff = vec3(0.299, 0.587, 0.114);
        // float luma[5];
        // for (int i = 0; i < 5; ++i) {
        //     luma[i] = dot(rgb[i], lumaCoeff);
        // }
        ...
        // vec2 dir;
        // dir.x = -((luma[1] + luma[2]) - (luma[3] + luma[4]));
        // dir.y =  ((luma[1] + luma[3]) - (luma[2] + luma[4]));

        // float dirReduce = max((luma[1] + luma[2] + luma[3] + luma[4]) * (0.25 * FXAA_REDUCE_MUL), FXAA_REDUCE_MIN);

        // float rcpDirMin = 1.0 / (min(abs(dir.x), abs(dir.y)) + dirReduce);
        // dir = clamp(dir * rcpDirMin * inverseResolution, vec2(-FXAA_SPAN_MAX), vec2(FXAA_SPAN_MAX));
