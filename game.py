...
class Game:
    ...
    def get_reward(self, paddle, action):
        ...
        reward = 0
        reward += c.reward_policy["time_reward"]

        reward += np.linalg.norm(acceleration) * c.reward_policy["acc_reward"]

        if (self.player_1_scored() and paddle.player == 1) or (self.player_2_scored() and paddle.player == 2):
            reward += c.reward_policy["goal"]

        relative_pos = self.puck.pos - paddle.pos
        dist_to_puck = np.linalg.norm(relative_pos)
        proximity_reward = ((c.settings['field_width'] - dist_to_puck) / c.settings['field_width']) * c.reward_policy["puck_proximity"]
        reward += proximity_reward

        goal_pos = h.goal_pos(2) if paddle.player == 1 else h.goal_pos(1)
        puck_to_goal_dist = np.linalg.norm(goal_pos - self.puck.pos)
        puck_to_goal_reward = ((c.settings['field_width'] - puck_to_goal_dist) / c.settings['field_width']) * c.reward_policy["goal_puck_proximity"]
        reward += puck_to_goal_reward

        pointless_reward = paddle.pointless_motion(acceleration) * c.reward_policy["pointless_motion"]
        reward += pointless_reward

        reward += self.puck.collect_shot_reward('shot_toward_goal', player=paddle.player) * c.reward_policy["shot_toward_goal"]

        reward += self.puck.collect_shot_reward('shot', player=paddle.player) * c.reward_policy["shot"]

        reward /= c.reward_policy["normalization"]
    ...
    def get_observation(self, player):
        if player == 1:
            obs = {
                ...
                "paddle_1_vel": self.scale(self.paddle1.vel, c.MAX_PADDLE_SPEED, c.MAX_PADDLE_SPEED),
                "paddle_2_vel": self.scale(self.paddle2.vel, c.MAX_PADDLE_SPEED, c.MAX_PADDLE_SPEED),
                "puck_vel":     self.scale(self.puck.vel, c.MAX_PUCK_SPEED, c.MAX_PUCK_SPEED),
            ...
            obs = {
                ...
                "paddle_1_vel": self.scale(self.paddle2.vel, c.MAX_PADDLE_SPEED, c.MAX_PADDLE_SPEED),
                "paddle_2_vel": self.scale(self.paddle1.vel, c.MAX_PADDLE_SPEED, c.MAX_PADDLE_SPEED),
                "puck_vel":     self.scale(self.puck.vel, c.MAX_PUCK_SPEED, c.MAX_PUCK_SPEED),
    ...
    def total_training_steps_left(self):
        return c.training_params['training_steps'] - self.total_steps
    ...
    def player_1_scored(self):
        ...
        return self.puck.pos[0] >= c.settings['field_width'] - self.puck.radius and self.puck.pos[1] > self.goal_top() and self.puck.pos[1] < self.goal_bottom()
    
    def player_2_scored(self):
        if c.settings['blocked_goals']:
            return False
                
        return self.puck.pos[0] <= self.puck.radius and self.puck.pos[1] > self.goal_top() and self.puck.pos[1] < self.goal_bottom()
...
def main(ai=False):
    ...
    latest_model_path, opponent_algorithm = h.get_latest_model_path_with_algorithm(c.training_params['base_path'], 'PPO')
